	...
	int opening;
		...
		if(path != NULL && pArrayListPassenger != NULL)
		{
			report = 0;
        		...
        		opening = file_serachPassengerBin(path , pArrayListPassenger); // 1 no encontro iguladad  - 0 hay igualdad de pasajeros

        			if(opening == 0)
        			{
        				file_writeBelowBinBis(path,  pArrayListPassenger);
